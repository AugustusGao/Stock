...
namespace ML.NetComponent.Http
{
    ...
    public class HttpHelper
    {
        ...
        public HttpResult GetHtml(HttpItem item)
        {
            ...
            result.Cookie = HttpCookieHelper.CookieListFormat(HttpCookieHelper.GetCookieList(result.Cookie));
            return result;
        }
        ...
        public async Task<HttpResult> GetHtmlAsync(HttpItem item)
        {
            ...
            try
            {
                ...
                using (response = (HttpWebResponse)r)
                {
                    GetData(item, result);
                }
            }
        ...
        public void GetAsyncHtml(HttpItem item, Action<HttpResult, Exception> action)
        {
            try
            {
                //准备参数
                SetRequest(item);
                //请求数据
                request.GetResponseAsync().ContinueWith((wr) =>
                {
                    HttpResult result = null;
                    Exception exception = null;
                    try
                    {
                        response = (HttpWebResponse)wr.Result;
        ...
        public void GetAsyncHtml(HttpItem item, object state, Action<HttpResult, object, Exception> callbackAction)
        {
            try
            {
                //准备参数
                ...
                request.GetResponseAsync().ContinueWith((wr) =>
                {
                    ...
                    finally
                    {
                        ...
                        {
                            callbackAction(result, state, exception);
                        }
                    }
        ...
        private void GetData(HttpItem item, HttpResult result)
        {
            ...
            if (response.Headers["Set-Cookie"] != null)
            {
                ...
                result.Cookie = result.Cookie.Replace("path=/,", "").Replace("path=/", "").Replace("Path=/,", "").Replace("Path=/", "");
            }//多个Cookie用下下次请求出错
            #endregion
        ...
        private void SetCookie(HttpItem item)
        {
            if (!string.IsNullOrEmpty(item.Cookie))
            {
                ...
                request.CookieContainer.SetCookies(uri, item.Cookie.TrimEnd(','));
                try
                {
            ...
            get
            {
                try
                {
                    if (Header != null && Header.Count > 0)
                    {
                        ...
                        {
                            string locationurl = Header["location"];
